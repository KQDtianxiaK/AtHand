...
在完成 V2 之后，AtHand 下一阶段不再适合继续沿着“功能更多的个人办公套件”扩张，而需要重新收束为更清晰的产品定义。

V3 的核心判断是：

- AtHand 不应该继续只是“带 AI 的工作台”，而应该转向“面向高杠杆个体的个人 AI 运营系统”。
- 第一目标用户不再是狭义的 CEO，而是同时管理多个 AI 会话、多个任务流、多个信息源的技术型个体创业者、独立开发者和小团队负责人。
- 产品主循环应收敛为：捕获需求、澄清约束、派发执行、观察状态、审批动作、沉淀记忆。
- AI 管控、全局 Assistant、timeline、审批流、长期记忆将成为核心；邮箱、新闻、打卡等模块则更适合作为连接器、信号源和派生视图，而不是并列一级产品。
- 未来 AtHand 的价值不在于“再做一个邮箱/待办应用”，而在于帮助用户持续运营自己的 AI 工作系统。

对应的信息架构也会从“应用分类导航”逐步转向“任务流导航”。建议的 V3 一级入口是：Today、Inbox、Missions、Runs、Memory，再把连接器与系统配置放到次级区域。

完整分析、产品原则和重组方案见：[docs/athand-v3-strategy.md](docs/athand-v3-strategy.md)。
